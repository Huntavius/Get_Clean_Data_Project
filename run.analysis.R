...
fileurl <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
     ...
     download.file(fileurl,temp)
          ## unzips the selected files to the data folder of your working directory
     ...
     head(act_melt)
          ...
          ## Recasts the act_melt data as a tidy dataset of means of each variable for each test
          ## subject
          act_melt_summary1 <- dcast(act_melt, Test_Subject ~ variable,mean)
     
          ## Recasts the act_melt data as a tidy dataset of means of each variable for each test
          ## subject
          act_melt_summary2 <- dcast(act_melt, Activity_name ~ variable,mean)
     
               ## Recasts the act_melt data as a tiday dataset of means for the two way interaction 
               ## between the variables and Activity or the variables and Test_Subject.  
               act_melt_summary3 <- dcast(act_melt, Test_Subject ~ variable*Activity_name,mean)
               act_melt_summary4 <- dcast(act_melt, Activity_name ~ variable*Test_Subject,mean)
               act_melt_summary5 <- dcast(act_melt, variable ~ Activity_name*Test_Subject,mean)
                     head(act_melt_summary5)
